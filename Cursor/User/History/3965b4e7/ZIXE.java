...
public class Main {
    ...
    private static void handleAdminMenu(Credentials credentials, String id, CourseCatalog courseCatalog) {
        List<String> adminInfo = credentials.getAdminInfo(id);
        // Make sure adminInfo has at least 3 elements before accessing them
        if (adminInfo.size() < 3) {
            System.out.println("Error: Insufficient admin information.");
            return;
        }
        
        Admin admin = new Admin(adminInfo.get(0), adminInfo.get(1), adminInfo.get(2));
